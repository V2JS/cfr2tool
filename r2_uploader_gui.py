...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, QLabel, 
                            ...
                            QMenu, QStackedWidget, QListWidget, QListWidgetItem)
...
class R2UploaderGUI(QMainWindow):
    ...
    def check_config_file(self):
        ...
        self.config_path = os.path.join(current_dir, 'config.env')
        if not os.path.exists(self.config_path):
            QMessageBox.warning(self, '配置文件缺失', 
                f'未找到 config.env 配置文件，请在以下路径创建该文件并填写配置：\n{self.config_path}')
            sys.exit(1)
